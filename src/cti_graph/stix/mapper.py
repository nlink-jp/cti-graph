"""STIX 2.1 objects → SQLite row conversion.

Each map_* method returns a dict suitable for repository upsert.
Objects outside scope return None (caller skips them).

Timestamps are stored as ISO 8601 strings (SQLite TEXT columns).
"""
...
class StixMapper:
    """Maps STIX 2.1 objects to the cti-graph SQLite schema."""
    ...
    def map_threat_actor(self, obj: dict) -> dict | None:
        ...
        return {
            ...
            "first_seen": _to_iso(obj.get("first_seen")),
            "last_seen": _to_iso(obj.get("last_seen")),
            "stix_modified": _to_iso(obj.get("modified")) or _now_iso(),
        }
    ...
    def map_vulnerability(self, obj: dict) -> dict | None:
        ...
        return {
            ...
            "published_date": _to_iso(obj.get("created")),
            "stix_modified": _to_iso(obj.get("modified")) or _now_iso(),
        }
    ...
    def map_observable(self, obj: dict) -> dict | None:
        ...
        return {
            ...
            "tlp": _tlp(obj),
            "first_seen": _to_iso(obj.get("valid_from")),
            "last_seen": _to_iso(obj.get("valid_until")),
            "stix_modified": _to_iso(obj.get("modified")) or _now_iso(),
        }
    ...
    def map_incident(self, obj: dict) -> dict | None:
        ...
        return {
            ...
            "name": obj["name"],
            "description": obj.get("description"),
            "occurred_at": _to_iso(obj.get("first_seen")),
            "resolved_at": _to_iso(obj.get("last_seen")),
    ...
    def map_incident_ttp_edges(self, obj: dict) -> list[dict]:
        """Generate IncidentUsesTTP rows from kill_chain_phases sequence."""
        if obj["type"] != "incident":
            return []
        incident_id = obj["id"]
        rows = []
    ...
    def map_relationship(self, obj: dict) -> tuple[str, dict] | None:
        """Map a STIX relationship to (table_name, row_dict). None if not applicable."""
        ...
        if rel_type == "uses" and "attack-pattern--" in dst:
            if src.startswith(("malware--", "tool--")):
                return "MalwareUsesTTP", {
                    ...
                    "first_observed": _to_iso(obj.get("start_time")),
                    "last_observed": _to_iso(obj.get("stop_time")),
                    "stix_id": stix_id,
                }
            return "Uses", {
                "actor_stix_id": src,
...
def build_followed_by_weights(
    ...
    """Derive FollowedBy(threat_intel) edges with 4-factor weight.

    weight = base_prob x activity_score x exploit_ease x ir_multiplier
    """
    ...
    all_ttps = {t for ttps in actor_ttps.values() for t in ttps}
    for ttp_id in all_ttps:
        relevant = [r for r in uses_rows if r.get("ttp_stix_id") == ttp_id]
        dated = [r for r in relevant if r.get("last_observed") is not None]
        if not dated:
            ttp_activity[ttp_id] = 0.5
        else:
            recent = sum(
                1
                for r in dated
                if _parse_ts(r["last_observed"]) is not None and _parse_ts(r["last_observed"]) >= cutoff_dt
            )
...
def build_ir_feedback_followed_by(
    ...
    Returns (followed_by_rows, ir_feedback_pairs).
    """
    ...
    for row in incident_ttp_rows:
        if row.get("sequence_order") is None:
            continue
        incident_sequences[row["incident_stix_id"]].append((row["sequence_order"], row["ttp_stix_id"]))
...
def _to_iso(val: Any) -> str | None:
    """Convert a timestamp value to ISO 8601 string for SQLite storage."""
    dt = _parse_ts(val)
    if dt is None:
        return None
    return dt.isoformat()
